...
class CategoryAdapter(
    ...
    private val changeSelectCategory: (Int) -> Unit
) :
    RecyclerView.Adapter<CategoryViewHolder>() { // hacermos una clase para el adaptador de la recycler view
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): CategoryViewHolder { // motara la vista para que poder ser mostrada, debe haber un vista previamente
        val view =
    ...
    override fun onBindViewHolder(
        ...
        position: Int
    ) { // tomara la vista creada por onCreateViweHolder para mostrarla
        holder.render(categories[position], changeSelectCategory)
    }
}